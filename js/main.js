...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (mobileMenuToggle && mainNav) {
        ...
        // Simplified event handler - works for both click and touch
        ...
        mobileMenuToggle.addEventListener('touchstart', function(e) {
            // Prevent double-firing by checking if already handled
    ...
    document.addEventListener('click', function(e) {
        if (mainNav && mainNav.classList.contains('active')) {
            ...
            if (mobileMenuToggle && (mobileMenuToggle.contains(e.target) || mobileMenuToggle === e.target)) {
                return;
            }
            // Don't close if clicking on overlay (it has its own handler)
            if (overlay && (overlay.contains(e.target) || overlay === e.target)) {
                return;
            }
            ...
            // Don't close if clicking on a navigation link (let it navigate)
            const clickedLink = e.target.closest('a');
            if (clickedLink && mainNav.contains(clickedLink) && !clickedLink.classList.contains('dropdown-toggle')) {
                return;
            }
    ...
    document.addEventListener('touchend', function(e) {
        if (mainNav && mainNav.classList.contains('active')) {
            ...
            // Don't close if touching on overlay (it has its own handler)
            if (overlay && (overlay.contains(e.target) || overlay === e.target)) {
                return;
            }
            ...
            if (link && mainNav.contains(link) && !link.classList.contains('dropdown-toggle')) {
                return;
            }
    ...
    navLinks.forEach(function(link) {
        // Use a small delay to allow navigation to start before closing menu
        link.addEventListener('click', function(e) {
            if (window.innerWidth < 1025) {
